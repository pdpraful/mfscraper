...
class DSPScraper(BaseAMCScraper, PDFExtractorMixin):
    def __init__(self):
        super().__init__(
            amc_name="DSP", 
            base_url="https://www.dspim.com/about-us/mandatory-disclosure/addendum"
        )
        self.factsheet_url = "https://www.dspim.com/downloads"
        ...
        for link in links:
            ...
            if href and '.pdf' in href.lower() and len(text) > 5:
                ...
                notices_data.append({
                    ...
                    'url': f"https://www.dspim.com{href}" if href.startswith('/') else href,
                    'type': notice_type,
                    'amc': self.amc_name
                })
                count += 1
    ...
    def scrape_aum(self, page: Page):
        ...
        if latest_factsheet_url.startswith('/'):
            latest_factsheet_url = f"https://www.dspim.com{latest_factsheet_url}"
        ...
        if pdf_path:
            with get_db_session() as db:
                ...
                for fund in funds:
                    keywords = [w for w in fund.name.split() if w.lower() not in ['dsp', 'fund', 'etf', 'fof', 'mutual', 'direct', 'growth', 'plan', 'fund of funds', 'global']]
                    if not keywords:
                        continue
