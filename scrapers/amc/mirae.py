...
class MiraeScraper(BaseAMCScraper, PDFExtractorMixin):
    def __init__(self):
        super().__init__(
            amc_name="Mirae", 
            base_url="https://www.miraeassetmf.co.in/downloads/statutory-disclosure/addendum"
        )
        self.factsheet_url = "https://www.miraeassetmf.co.in/downloads/factsheets"
        ...
        for link in links:
            ...
            if href and '.pdf' in href.lower() and len(text) > 5:
                ...
                notices_data.append({
                    ...
                    'url': f"https://www.miraeassetmf.co.in{href}" if href.startswith('/') else href,
                    'type': notice_type,
                    'amc': self.amc_name
                })
                count += 1
    ...
    def scrape_aum(self, page: Page):
        ...
        if latest_factsheet_url.startswith('/'):
            latest_factsheet_url = f"https://www.miraeassetmf.co.in{latest_factsheet_url}"
        ...
        if pdf_path:
            with get_db_session() as db:
                funds = db.query(Fund).filter(
                    Fund.amc.ilike("%Mirae%"),
                    Fund.is_international == True
                ).all()
                
                for fund in funds:
                    keywords = [w for w in fund.name.split() if w.lower() not in ['mirae', 'asset', 'fund', 'etf', 'fof', 'mutual', 'direct', 'growth', 'plan', 'fund of funds', 'global']]
